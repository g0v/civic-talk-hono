...
- **站內事實查核 API**：`/api/fact-check` 透過 Cloudflare Service Binding 呼叫沒有公開 route 的 `fact-check-core`；要求同源 `Origin` 與短效 `X-Civic-Talk-Token`，`POST` 另外要求登入；每位登入使用者每 60 秒最多 60 次；這是防洪而非配額。
- **素材投稿先做事實查核**：素材表單目前仍直接呼叫 `https://check.vtaiwan.tw/api/fact-check`；站內 `/api/fact-check` 已存在，但前端切換待後續 PR。查核完成且結果允許前不顯示張貼按鈕，查核錯誤、社群守則封鎖、查核結果不完整或事實性不足都不能張貼。送出時仍由 Civic Talk API 執行登入、條款與投稿安全審查。
- **事實查核的已知風險**：已實作 D1 固定視窗速率限制，但 D1 故障時 fail-open；已登入帳號仍可取得自己的短效 token 轉送請求，並在 15 分鐘有效期內重放；邊緣層 rate limit／Turnstile 仍是最外層的防線。
