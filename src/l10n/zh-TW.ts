export const messages = {
  ...
  terms_s3_p1: '<strong>建立議題、提交素材、發表意見、送出說明頁／彙整</strong>均為具名行為。提交後，您提交當下的 OAuth 顯示名稱快照將公開附在對應內容上，所有訪客均可查看；日後修改帳號名稱不會追溯變更既有內容的署名。',
  terms_s3_p2: '提交時，您可以勾選「公開我的 email」選項。平台會保存提交當下的 email 快照；若選擇公開，該快照將隨顯示名稱公開顯示，若未選擇則不會出現在公開 API、頁面或 SSR 原始碼，僅供具管理權限者追溯濫用及在必要時配合調查（見「執法配合」）。',
  ...
  terms_s7_alert_body: '若有使用者濫用平台（包含但不限於散布不實資訊、騷擾、非法內容），平台保留依法配合主管機關（包含警察機關）調查、提供相關使用記錄與帳號資料的權利。<strong>上述資料包括但不限於：帳號 ID、OAuth 顯示名稱、email（無論使用者是否選擇公開）、提交時間與 IP 位址等技術資料。</strong>若有必要，平台將依法律程序配合提供前述資料。',
  terms_s8_h: '內容授權',
  terms_s8_p: '您在本平台發布的意見摘要，以 <strong>CC-BY-SA 4.0</strong> 授權公開。素材的授權依各來源的授權條款為準；提交者須自行確認授權範圍（見「素材授權規範」）。',
  terms_s9_h: '免責聲明',
  terms_s9_p: '本平台按「現狀」提供服務，不提供任何明示或暗示的保證。平台不承擔因使用服務（包含素材內容、意見觀點）而產生的任何直接或間接損失。平台上的素材與意見僅代表提交者個人立場，不代表 vTaiwan 或平台的官方立場。',
  terms_s10_h: '條款修改',
  ...
  privacy_s3_p1: '為確保內容品質與可追溯性，上述行為提交後，您提交當下的 OAuth 顯示名稱將公開附在對應的議題、素材或意見上，所有訪客均可查看。日後修改帳號名稱不會追溯變更既有內容的署名。',
  privacy_s3_p2: '若 OAuth 帳號沒有提供顯示名稱，平台會顯示固定的「未提供顯示名稱」提示，不會以 email 代替。',
  ...
  privacy_s3_p4: 'user ID 是 Better Auth 產生的平台內部識別碼，不是 Google 或 GitHub 的公開帳號名稱；一律不公開顯示。議題、素材、意見與說明頁都會公開顯示提交當下的顯示名稱；email 僅在該筆內容勾選公開時才顯示。',
  privacy_s4_h: '資料使用目的',
  ...
  privacy_s5_alert_body: '若有使用者濫用平台（包含但不限於散布不實資訊、騷擾他人、非法內容等），平台保留依法配合主管機關（包含警察機關）調查、提供相關使用記錄的權利。<strong>上述記錄包括但不限於：帳號 ID、OAuth 顯示名稱、email（無論使用者是否選擇公開）、提交時間與 IP 位址等技術資料。</strong>',
  privacy_s6_h: '資料安全',
  ...
  privacy_s7_li2: '<strong>更正權</strong>：若您的 OAuth 顯示名稱已更新，下次登入後提交的內容將反映新名稱；歷史記錄中的顯示名稱為提交當下的快照，平台不提供追溯修改。',
  privacy_s7_li3: '<strong>刪除與匿名化權</strong>：要求刪除帳號或匿名化內容中的作者快照。由於內容資料與共用認證資料庫分開保存，刪除登入帳號不會自動刪除既有內容的作者快照；平台收到請求後會另行處理，惟依法或為處理爭議所需者可能在必要期間內保留。',
