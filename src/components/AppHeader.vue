...
<template>
  <header class="sticky top-0 z-[100] px-4 pt-3 bg-transparent">
    ...
    >
      ...
      <nav class="hidden items-center gap-0.5 md:flex">
        ...
        <template v-if="authState === 'signed-in'">
          ...
          <span
            class="max-w-[10rem] truncate text-[13px] text-vt-fg-2"
      ...
      <div class="flex items-center gap-2 md:hidden">
        ...
        <button
          type="button"
          class="flex h-9 w-9 items-center justify-center rounded-lg hover:bg-black/5"
          :aria-expanded="showMenu"
          aria-label="選單"
          @click="showMenu = !showMenu"
        >
          <!-- 關閉（X）圖示 -->
          <svg v-if="showMenu" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
    ...
    <div
      v-if="showMenu"
      class="mx-auto mt-2 max-w-[980px] rounded-xl border border-white/75 bg-white/97 px-5 py-4 shadow-md backdrop-blur-[20px] md:hidden"
    >
      <!-- 導航連結 -->
      <nav class="mb-4 flex flex-col gap-1">
        <a
          v-if="backHref"
          :href="backHref"
          class="rounded-lg px-3 py-2.5 text-[15px] text-vt-fg-1 no-underline hover:bg-black/5"
          @click="closeMenu"
        >
          ← {{ backText }}
        </a>
    ...
    <div
      ...
      class="mx-auto mt-2 hidden max-w-[980px] rounded-xl border border-white/75 bg-white/95 p-4 shadow-md backdrop-blur-[20px] md:block"
    >
      <p class="mt-0 mb-3 text-sm text-muted">{{ t('login_panel_desc') }}</p>
      <SignInButtons :callback-url="loginCallbackUrl" />
    </div>
